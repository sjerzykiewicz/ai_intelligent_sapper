...
class Game:
    ...
    def __init__(self):
        ...
        self.screen_drawer = ScreenDrawer(self.sapper, self.screen, self.clock, self.grass_surf, self.rects, self.landmine_surf, self.flag_surf, self.BLOCK_SIZE, self.WINDOW_WIDTH, self.WINDOW_HEIGHT, self.is_landmine_here, self.occupied_blocks, self.fence)
    ...
    def _create_fence(self):
        ...
        for y in range(
            ...
            rect = self.fence_vertical.get_rect(topleft=(self.WINDOW_WIDTH - self.BLOCK_SIZE, y))
            fence.append([self.fence_vertical, rect])
        ...
        for x in range(
            ...
            rect = self.fence_horizontal.get_rect(topleft=(x, self.WINDOW_HEIGHT - self.BLOCK_SIZE))
            fence.append([self.fence_horizontal, rect])
        ...
        rect = self.fence_corner_2.get_rect(topleft=(self.WINDOW_WIDTH - self.BLOCK_SIZE, 0))
        fence.append([self.fence_corner_2, rect])
        rect = self.fence_corner_3.get_rect(topleft=(0, self.WINDOW_HEIGHT - self.BLOCK_SIZE))
        fence.append([self.fence_corner_3, rect])

        rect = self.fence_corner_4.get_rect(topleft=(self.WINDOW_WIDTH - self.BLOCK_SIZE, self.WINDOW_HEIGHT - self.BLOCK_SIZE))
